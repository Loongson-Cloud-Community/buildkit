...
FROM lcr.loongnix.cn/minio/minio:${MINIO_VERSION}-alpine3.21 AS minio
FROM lcr.loongnix.cn/minio/mc:${MINIO_MC_VERSION}-alpine3.21 AS minio-mc

# xx is a helper for cross-compilation
FROM --platform=$BUILDPLATFORM  lcr.loongnix.cn/tonistiigi/xx:${XX_VERSION} AS xx

# golatest is alias for Go base image
FROM --platform=$BUILDPLATFORM lcr.loongnix.cn/library/golang:${GO_VERSION}-alpine AS golatest

# git stage is used for checking out remote repository sources
FROM --platform=$BUILDPLATFORM lcr.loongnix.cn/library/alpine:${ALPINE_VERSION} AS git
...
FROM --platform=$BUILDPLATFORM lcr.loongnix.cn/library/alpine:${ALPINE_VERSION} AS cni-plugins
...
FROM --platform=$BUILDPLATFORM lcr.loongnix.cn/library/alpine:${ALPINE_VERSION} AS releaser
...
FROM lcr.loongnix.cn/library/alpine:${ALPINE_VERSION} AS buildkit-export
...
FROM lcr.loongnix.cn/library/alpine:${ALPINE_VERSION} AS rootless
